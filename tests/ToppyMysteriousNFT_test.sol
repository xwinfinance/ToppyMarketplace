//SPDX-License-Identifier: UNLICENSED
 ...
 import "../contracts/ToppyMysteriousNFT.sol";

 // File name has to end with'_test.sol', this file can contain more than one testSuite contracts
 /// Inherit 'ToppyMysteriousNFT' contract
 contract ToppyMysteriousNFTTest is ToppyMysteriousNFT {
    ...
    function setLunchVenue() public {
        Assert.equal(addVenue('good Cafe') , 1 ,'Should be equal to 1');
        Assert.equal(addVenue('bad Cafe') , 2,'Should be equal to 2');
    }

    /// Try to add lunch venue as a user other than manager.This should fail
    /// #sender: account-1
    function setLunchVenueFailure() public {
        try this.addVenue('random Cafe') returns(uint v) {
    ...
    function beginVoting() public {
        Assert.ok(beginVote(), "Unknown error");
    }
    ...
    function lunchVenueTest() public {
        Assert.equal(votedVenue ,'bad Cafe','Selected venue should be bad Cafe');
    }
 ...
 contract LunchVenueTest2 is LunchVenue {
    ...
    /// initialize venues and friends as account-0
    /// #sender doesn't need to be specified explicitly for account-0
    ...
    /// Vote before voting open
    /// #sender: account-1
    function voteBeforeBegin() public {
        try this.doVote(2) returns(bool v) {
            Assert.ok(false ,'Method execution should fail');
        } catch Error(string memory reason) {
            // Compare failure reason , check if it is as expected
            Assert.equal(reason ,'Can vote only while voting is open.','Failed with unexpected reason');
    ...
    /// cannot add friend or add venue once voting has begun
    /// #sender doesn't need to be specified explicitly for account-0
    function addVenueFail() public {
        try this.addVenue('random Cafe') returns(uint v) {
            Assert.ok(false ,'Method execution should fail');
        } catch Error(string memory reason) {
            // Compare failure reason , check if it is as expected
            Assert.equal(reason ,'Can use only when voting has not begun.','Failed with unexpected reason');
        } catch(bytes memory /* lowLevelData */) {
            Assert.ok(false ,'Failed unexpected');
        }
    }
    ...
    /// Verify lunch venue is set correctly
    function cancelTest() public {
        Assert.equal(cancelVote(), true, "Unknown error");
        Assert.equal(status ,'This contract has been cancelled','Manager should have cancelled the contract');
    }
 }